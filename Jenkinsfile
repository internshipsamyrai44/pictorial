...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3682
        NAMESPACE = "pictorial-work"
        REGISTRY_HOSTNAME = "internshipsamyrai44"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "pictorial"
        DEPLOYMENT_NAME = "pictorial-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'pictorial-work') {
